...
from app.models.commerce import ShoppingCart, CartItem, Order, OrderItem, OrderStatus
...
def update_cart_item(db: Session, buyer_id: int, item_id: int, update_in: CartItemUpdate):
    cart = get_or_create_cart(db, buyer_id)
...
def create_order_from_cart(db: Session, buyer_id: int, order_in: OrderCreate):
    ...
    for item in cart.items:
        ...
        if not product or product.status != "PUBLISHED" or product.stock_quantity < item.quantity:
            raise HTTPException(status_code=400, detail=f"Product {product.name if product else item.product_id} is unavailable or out of stock")
...
def get_supplier_orders(db: Session, supplier_id: int):
    orders = db.query(Order).join(OrderItem).filter(OrderItem.supplier_id == supplier_id).order_by(Order.created_at.desc()).all()
    return orders

def update_order_status(db: Session, order_id: int, supplier_id: int, status: str):
    order = db.query(Order).join(OrderItem).filter(Order.id == order_id, OrderItem.supplier_id == supplier_id).first()
    if not order:
        raise HTTPException(status_code=404, detail="Order not found")
    order.status = status
    db.commit()
