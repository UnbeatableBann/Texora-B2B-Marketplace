...
class SearchCriteria(BaseModel):
    category: str | None = Field(
        None, description="Product category like Cotton, Denim, Silk"
    )
    material: str | None = Field(
        None, description="Material composition like 100% Cotton"
    )
    color: str | None = Field(None, description="Color of the fabric")
    max_price: float | None = Field(None, description="Maximum price acceptable")
    quantity: int | None = Field(None, description="Quantity required")
    quantity_unit: str | None = Field(
        None, description="Unit of quantity like meter, kg"
    )


class AIIntent(BaseModel):
    intent: str = Field(
        description="One of: PRODUCT_SEARCH, GENERAL_MARKETPLACE_HELP, CLARIFICATION"
    )
    criteria: SearchCriteria | None = Field(
        None, description="Structured criteria if intent is PRODUCT_SEARCH"
    )
    clarification_question: str | None = Field(
        None, description="Question to ask if intent is CLARIFICATION"
    )
...
class AIProvider:
    ...
    def search_products(
        self, db: Session, criteria: SearchCriteria
    ) -> list[dict[str, Any]]:
