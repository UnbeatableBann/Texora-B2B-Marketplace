...
from app.schemas.commerce import ShoppingCartRead, CartItemCreate, CartItemUpdate, OrderRead, OrderCreate, OrderUpdate
...
def update_cart_item(item_id: int, update_in: CartItemUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    if current_user.role != "buyer":
...
def checkout(order_in: OrderCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    if current_user.role != "buyer":
...
def update_order(order_id: int, order_update: OrderUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    return commerce_repo.update_order_status(db, order_id, current_user.id, order_update.status)
