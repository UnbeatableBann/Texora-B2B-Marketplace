...
def google_auth(
    auth_data: GoogleAuth, db: Session = Depends(get_db)
):
    try:
        client_id = os.environ.get("GOOGLE_CLIENT_ID")
        # In dev, we might not have client_id set strictly, but google auth verifies aud
        idinfo = id_token.verify_oauth2_token(auth_data.credential, google_requests.Request(), client_id)
        ...
        if not user:
            ...
            if user:
                if user.auth_provider != "google":
                    raise HTTPException(status_code=400, detail="Email already registered with password. Please login with your password.")
            else:
                user = User(
                    email=email,
                    full_name=name,
