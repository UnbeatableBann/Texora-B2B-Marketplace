...
def read_products(
    ...
    if (
        current_user
        and current_user.role == "supplier"
        and current_user.id == supplier_id
    ):
        include_drafts = True
    return catalog_repo.get_products(
        db,
...
def get_trending_products(limit: int = 4, db: Session = Depends(get_db)):
    return (
        db.query(catalog_repo.Product)
        .filter(catalog_repo.Product.status == "PUBLISHED")
        .order_by(catalog_repo.Product.id.desc())
        .limit(limit)
        .all()
    )
...
def get_recommended_products(
    ...
    query = db.query(catalog_repo.Product).filter(
    ...
    if current_user and current_user.role == "buyer":
        ...
        if buyer_profile and buyer_profile.preferences:
            preferred_categories = buyer_profile.preferences.get(
                "product_categories", []
            )
            if preferred_categories:
                query = query.join(Category).filter(
                    Category.name.in_(preferred_categories)
                )
    ...
    if len(results) < limit:
        ...
        fallback_query = (
            db.query(catalog_repo.Product)
            .filter(catalog_repo.Product.status == "PUBLISHED")
            .order_by(catalog_repo.Product.price)
            .limit(limit)
        )
...
def create_product(
    ...
    if current_user.role != "supplier":
        raise HTTPException(
            status_code=403, detail="Only suppliers can create products"
        )
...
def update_product(
    ...
    if current_user.role != "supplier":
        raise HTTPException(
            status_code=403, detail="Only suppliers can update products"
        )
    db_product = catalog_repo.get_product(db, product_id)
    if db_product is None:
        raise HTTPException(status_code=404, detail="Product not found")
...
def delete_product(
    ...
    if current_user.role != "supplier":
        raise HTTPException(
            status_code=403, detail="Only suppliers can delete products"
        )
    db_product = catalog_repo.get_product(db, product_id)
    if db_product is None:
        raise HTTPException(status_code=404, detail="Product not found")
