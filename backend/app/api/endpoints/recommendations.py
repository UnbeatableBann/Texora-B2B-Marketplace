...
from app.schemas.catalog import ProductRead, RecommendationItem, RecommendationResponse
...
def score_product(product: Product, preferences: dict[str, Any]) -> tuple[float, str]:
    ...
    preferred_categories = [
        c.lower() for c in preferences.get("product_categories", [])
    ]
    product_category = product.category.name.lower() if product.category else ""
    if product_category and any(
        c in product_category or product_category in c for c in preferred_categories
    ):
...
def get_recommendations(
    ...
    if not current_user or current_user.role != "buyer":
        ...
        sorted_candidates = sorted(
            candidates, key=lambda p: p.price, reverse=True
        )  # Mock trending
        items = []
    ...
    buyer_profile = (
        db.query(BuyerProfile).filter(BuyerProfile.user_id == current_user.id).first()
    )
    ...
    for product in candidates:
        ...
        if score > 0:
            scored_products.append(
                {"product": product, "score": score, "reason": reason}
            )
    ...
    for item in scored_products:
        ...
        if (
            list(seen_categories).count(cat_id) >= 3
            or list(seen_suppliers).count(sup_id) >= 3
        ):
            continue
    ...
    if not final_recommendations:
        for product in candidates[:limit]:
            final_recommendations.append(
                {"product": product, "score": 0, "reason": "Explore popular fabrics"}
            )
